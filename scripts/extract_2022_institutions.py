#!/usr/bin/env python3
"""
Extract institution-level data from 2022 approved budget bill addendum.
Uses the same segmentation approach as 2020/2021: find all XX-XXX codes and map to areas.
"""
...
PDF_FILE = PROJECT_DIR / "data/landing/budget_bills_approved/2022/addendum/bill_2022_approved_addendum.pdf"
LAYOUT_TEXT = Path("/tmp/addendum_2022_layout.txt")
OUTPUT_JSON = Path("/tmp/institutions_2022_complete.json")

def extract_layout_text():
    """Extract text from PDF using pdftotext with layout preservation."""
    print("Extracting text from 2022 PDF...")
    subprocess.run(
...
def main():
    print("=" * 80)
    print("2022 Institution-Level Extraction")
    print("=" * 80)
    ...
    code_file = Path("/tmp/institution_data_2022_code.py")
    code_lines = ['INSTITUTION_DATA_2022_COMPLETE = {']
