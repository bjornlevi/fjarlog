...
Uses the same segmentation approach as 2020-2024: find all XX-XXX codes and map to areas.
"""
...
def extract_layout_text():
    ...
    print("Extracting text from 2025 PDF (pages 10-60)...")
    subprocess.run(
        ["pdftotext", "-layout", "-f", "10", "-l", "60", str(PDF_FILE), str(LAYOUT_TEXT)],
        check=True
    )
