#!/usr/bin/env python3
"""
Extract institution-level data from 2024 approved budget bill addendum.
...
PDF_FILE = PROJECT_DIR / "data/landing/budget_bills_approved/2024/addendum/bill_2024_approved_addendum.pdf"
LAYOUT_TEXT = Path("/tmp/addendum_2024_layout.txt")
OUTPUT_JSON = Path("/tmp/institutions_2024_complete.json")

def extract_layout_text():
    """Extract text from PDF using pdftotext with layout preservation."""
    print("Extracting text from 2024 PDF (pages 11-50)...")
    subprocess.run(
...
def main():
    print("=" * 80)
    print("2024 Institution-Level Extraction")
    print("=" * 80)
    ...
    code_file = Path("/tmp/institution_data_2024_code.py")
    code_lines = ['INSTITUTION_DATA_2024_COMPLETE = {']
